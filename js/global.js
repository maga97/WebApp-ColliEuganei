...
function validaFormUtente(validazionePassword, alertErrore, form) {
  ...
  anagrafica.each(function() {
    ...
    if (testo.length === 0) {
      notificaErrore(
        $(this),
        "Campo " + $(this).attr("name") + " obbligatorio",
        $(".alert.errore"),
        $("form")
      );
      formValido = false;
    ...
    if (/\d+/.test(testo)) {
      notificaErrore(
        ...
        "Il campo " +
          $(this).attr("name") +
          " non pu&ograve; contenere numeri ",
        $(".alert.errore"),
        $("form")
      );
      formValido = false;
  ...
  if (/[^\s@]+@[^\s@]+\.[^\s@]+/.test(email.val().trim()) === false) {
    notificaErrore(
      email,
      "Inserire un'<span xml:lang='en'> email </span> valida",
      alertErrore,
      form
    );
    formValido = false;
...
function validaPassword(password, password2, divErrore, formErrore) {
  ...
  if (password.val().trim().length === 0) {
    notificaErrore(
      password,
      'Inserire una nuova <span xml:lang="en"> password </span> valida',
      divErrore,
      formErrore
    );
    ...
    notificaErrore(
      password2,
      'Si prega di ripetere la nuova <span xml:lang="en"> password </span>',
      divErrore,
      formErrore
    );
    passwordValide = false;
    ...
    notificaErrore(
      password2,
      'Le <span xml:lang="en"> password </span> non combaciano',
      divErrore,
      formErrore
    );
    passwordValide = false;
