...
\begin{document}
	...
	\begin{titlepage}
		...
		\vfill
		{
			...
			{\small Credenziali per l'amministrazione:\\username: admin@admin.it, password: admin\\Credenziali utente registrato: utente@utente.it, password: utente}
			\vfill
			Indirizzo email del referente: giulio.piva.1@studenti.unipd.it\\
			\vfill
		}
	\end{titlepage}
	\pagenumbering{roman}
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="it" lang="it">
	...
	\subsubsection{Form}
	Gli elementi di input dei form obbligatori sono stati marcati \texttt{required="required"} questo consente ai browser recenti di verificare che il contenuto di input sia stato definito. La verifica viene sempre e comunque eseguita dal server, ogni qualvolta l'utente inserisce dei dati nell'applicazione. Inoltre, si è fatto uso solo del valore "text" per l'attributo value degli input, questo per garantire la retro-compatibilità con tutti i browser.
	Infatti, i controlli sui tipi di dato sono effettuati da JS, se abilitato, e da PHP in qualsiasi circostanza. Si è fatta questa scelta considerata la mole di browser che non supportano questi tipi di dato ancora oggi. Ad esempio Safari sviluppato da Apple, non ha ad oggi il supporto al tag di input con valore \texttt{date}. Il gruppo ha valutato la possibilità di utilizzare un datepicker, ma tale soluzione richiede JavaScript sempre abilitato e ciò non è sempre possibile.
	...
	Infine, è stato creato un file \texttt{print.css} utilizzato per la stampa, rimuove tutti i contenuti non necessari e impagina le informazioni per meglio adattarle a un foglio A4.
	 ...
	 Sono stati aboliti inoltre le proprietà dei vendor (webkit di Apple, -moz di Firefox e ms di Microsoft) cercando di utilizzare proprietà compatibili con tutti i browser e con lo stesso risultato.
...
return password_verify($password, $pass[0]);
	...
	Si è fatto uso del plugin WCAG contrast checker e si è riscontrato un valore di almeno "AAA" per tutti i colori all'interno del sito.
	Dovrebbe essere semplice per utenti con disabilità, ad esempio affetti da daltonismo ad usufruire dei contenuti del sito in maniera semplice.
	...
	\begin{itemize}
		...
		W3C.
		\item  Le parole in lingua inglese sono state racchiuse in un tag \\ \texttt{<span xml:lang="en" lang="en">contenuto</span>}
...
%		\item Erano stati utilizzati i tag \texttt{role} dove necessario seguendo i dettami del WCAG, per fornire un modo per identificare gli elementi in maniera più specifica ove necessario. In particolare, questa soluzione si è rivelata particolarmente utile in quanto, nonostante l'uso di HTML 5 che presenta dei tag specifici, ad esempio \texttt{<nav>} per il menu di navigazione, l'attributo role garantisce l'accessibilità e contemporaneamente la retro-compatibilità con i browser meno recenti.
%		Inoltre è stato evitato l'utilizzo di tabelle per realizzare il layout del sito.
		\item \uppercase{è} stato evitato l'utilizzo di tabelle per realizzare il layout del sito, e in generale all'interno del sito;
...
%			\item \texttt{role="alert"}: per segnalare appunto che si tratta di un messaggio interattivo, che è frutto di un'operazione eseguita dall'utente.
	...
	sito da vari dispositivi. Alla luce di questa considerazione il sito adotta un layout responsive grazie alla creazione di file css appositi (mobile e stampa). Il sito è stato testato su più dispositivi, browser e sistemi operativi possibili in modo tale da avere un feedback più completo possibile sulle scelte adottate (Vedere la sezione Validazione e Test per i dettagli). Un approccio non seguito all'inizio, ma che  è stato fondamentale è il \textit{mobile first}: utilizzando gli strumenti di sviluppo di Google Chrome per testare il sito prima per i telefoni cellulare, e poi adattarlo per PC desktop.
	Per verificare che il sito si adattasse correttamente ai vari dispositivi si è fatto uso degli strumenti sviluppatore forniti dai browser. Ad esempio abbiamo controllato che il contenuto informativo fosse fruibile da un iPhone, che solitamente richiede media query con dimensioni specifiche:
		\begin{figure}[H]
		\centering
		\includegraphics[scale=0.5]{images/iphone6.png} 
		\caption{Google Chrome, strumenti sviluppatori verifica fruibilità dei contenuti su iPhone}
		\end{figure}
...
Anche il bottone "back-to-top" che permette di tornare all'inizio della pagina è stato realizzato tramite un'ancora per garantire la migliore navigazione in qualsiasi circostanza.
	...
	Per la verifica automatica dell'accessibilità sono stati utilizzati due strumenti: \begin{itemize}
		...
		\item WCAG Contrast Checker, plugin per Google Chrome utilizzato per verificare che il contrasto dei colori avesse valore "AAA".
	\end{itemize}
	Come impostazione è stato utilizzato "WCAG 2.0 (Level AAA)" in tutti i validatori, e tutti i	potenziali problemi che vengono segnalati raccolgono tutti elementi che il validatore non sa come controllare e quindi viene richiesta una supervisione umana, come ad esempio ogni \texttt{alt}. \\
	...
	Abbiamo utilizzato Total Validator Basic per validare ogni pagina. Gli errori incontrati sono stati risolti, ma rimane un warning sollevato in ogni pagina  e riguardante il link interno per saltare al contenuto, precisamente "W874 Add a skip navigation link as the rst link on the page", che viene sollevato in quanto il testo non  è "Skip to main content", bensì "Vai al contenuto", in quanto il sito  e mirato ad utenti italiani. Da precisare che oltre alla presenza del link per andare al contenuto  e disponibile anche un link per tornare ad inizio pagina, inserito per facilitare la navigazione da dispositivi tascabili.
	...
	Sono state testate le risoluzioni di 1080p e 2160p con i browser:
	\begin{itemize}
	...
	\subsection{MacOS}
	Sono state testate le risoluzioni di 1080p, 2160p con i browser:
	\begin{itemize}
		\item Safari
		\item Firefox Quantum
		\item Google Chrome
	...
	Di seguito sono elencati i vari dispositivi mobili sui quali il sito è stato testato, oltre ad alcuni simulati da Google Chorme:
	\begin{itemize}
